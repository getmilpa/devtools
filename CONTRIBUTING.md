# Contributing to Milpa DevTools

Thanks for your interest in contributing! Milpa DevTools is the `coa` generate-verify-inspect
developer loop of the Milpa framework — deterministic controller/entity scaffolding (the Make
engine), boot-time doctors (framework health, plugin-manifest, capability-graph), and the
convention verifiers that check generated code against the framework's own runtime rules. It
builds on `milpa/core`, with no product coupling.
...
- **Respect the tier boundary.** Milpa DevTools depends on `milpa/core`, never the reverse.
  `doctrine/orm` is the one deliberate exception — `Verify\EntityVerifier` reflects real Doctrine
  ORM attributes, so that dependency is genuine and documented, not incidental. Do not introduce a
  dependency on a concrete container, an HTTP kernel, or any product/plugin code. The `Make`/
  `Verify` generators and checkers target the framework's *documented* conventions
  (`Milpa\app\Providers\BaseController`, `HttpResponse`, …) as strings/FQCNs a host app carries —
  never as a `use` import of a specific host application's classes.
...
The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/tool-runtime`, `milpa/devtools`, …)
